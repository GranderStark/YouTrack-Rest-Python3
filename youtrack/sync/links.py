    ...
    def importLinks(self, links):
        ...
        for link in links:
            if link.target not in self.created_issue_ids:
                print((self.header + ' failed to import link ' + self._getPrettyLink(
                    link) + ' to ' + self.target_name + ' because ' + link.target + ' was not imported'))
            elif link.source not in self.created_issue_ids:
                print((self.header + 'failed to import link ' + self._getPrettyLink(
                    link) + ' to ' + self.target_name + ' because ' + link.source + ' was not imported'))
            else:
...
class LinkSynchronizer(object):
    ...
    def collectLinksToSyncById(self, master_issue_id, slave_issue_id):
        ...
        to_master_links = set(
            [self._convertSlaveLinkForMaster(link) for link in slave_links if self.check_slave_link(link)]) - set(
            master_links)
        to_slave_links = set(
            [self._convertMasterLinkForSlave(link) for link in master_links if self.check_master_link(link)]) - set(
            slave_links)
...
class IssueBinder(object):
    ...
    def slaveIssueIdToMasterIssueId(self, slave_issue_id):
        return self.s_to_m[str(slave_issue_id)]

    def masterIssueIdToSlaveIssueId(self, master_issue_id):
        return self.m_to_s[str(master_issue_id)]
    ...
    def checkSlaveId(self, id):
        return id in list(self.s_to_m.keys())

    def checkMasterId(self, id):
        return id in list(self.m_to_s.keys())
