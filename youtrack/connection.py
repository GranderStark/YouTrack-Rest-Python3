...
class Connection(object):
    ...
    def _login(self, login, password):
        ...
        response, content = self.http.request(
            ...
            uri=self.baseUrl + '/hub/auth/login',
            method='POST',
            body=body,
    ...
    def __get_illegal_xml_chars_re():
        ...
        return re.compile('[%s]' % ''.join(_illegal_ranges))
    ...
    def getAttachmentContent(self, url):
        f = urllib.request.urlopen(urllib.Request(self.url + url, headers=self.headers))
        return f
    ...
    def addUserRoleToGroup(self, group, userRole):
        url_group_name = urlquote(group.name)
        url_role_name = urlquote(userRole.name)
    ...
    def createRole(self, role):
        ...
        if hasattr(role, 'description'):
            url_role_dscr = urlquote(role.description)
    ...
    def changeRole(self, role, new_name, new_description):
        url_role_name = urlquote(role.name)
        url_new_name = urlquote(new_name)
        url_new_dscr = urlquote(new_description)
    ...
    def addValueToBundle(self, bundle, value):
        ...
        if bundle.get_field_type() != "user":
            request = "/admin/customfield/%s/%s/" % (
                self.bundle_paths[bundle.get_field_type()], urlquote(bundle.name))
